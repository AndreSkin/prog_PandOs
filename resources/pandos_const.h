...
/*Massimo numero di sub-devices*/
#define MAX_SUB_DEV 49
/*Creato leggendo la definizione di STCK(T) (p.133 pops)*/
#define NOW ((* ((cpu_t *) TODLOADDR)) / (* ((cpu_t *) TIMESCALEADDR)))
/*Timer converter (p.133 pops)*/
#define TIMECONV(T)  ((T) * (*((cpu_t *) TIMESCALEADDR)))
/*Interrupting Devices Bit Map*/
#define INT_DEV_BM 0x10000040

/*Indirizzo di inizio dei device registers*/
#define BEGIN_DEVREG 0x10000054

/*Indirizzo di inizio della swap pool, secondo quanto riportato nel paragrafo 4.4.1 della student guide*/
#define SWAP_POOL_STARTING_ADDRESS (0x20000000 + (32 * PAGESIZE))
